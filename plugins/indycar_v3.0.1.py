...
def _fetch(url):
    r=requests.get(url,timeout=9,headers={"User-Agent":"RackDash-IndyCar/3.0.1","Accept":"text/html,application/xhtml+xml"}); r.raise_for_status(); return r.text
...
def _headlines():
    ...
    try:
        r=requests.get(INDYCAR_NEWS_RSS,timeout=7,headers={"User-Agent":"RackDash-IndyCar/3.0.1"}); r.raise_for_status(); root=ET.fromstring(r.content); rows=[]
...
def _weather(e):
    ...
    try:
        r=requests.get("https://api.open-meteo.com/v1/forecast",params={"latitude":e["lat"],"longitude":e["lon"],"daily":"weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,wind_speed_10m_max","temperature_unit":"fahrenheit","wind_speed_unit":"mph","timezone":"auto","start_date":e["date"],"end_date":e["date"]},timeout=7,headers={"User-Agent":"RackDash-IndyCar/3.0.1"}); r.raise_for_status(); d=r.json().get("daily") or {}
